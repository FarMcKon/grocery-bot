...
def log_sms_message(phone_number, message_content, direction, user_id=None, list_id=None):
    message = {
...
def sms_webhook():
    ...
    try:
        ...
        phone_number = data.get('From', '').replace('+1', '').replace('-', '').replace(' ', '')
...
def process_sms_command(user, command, params):
    ...
    try:
        ...
        if command == 'add' and params:
                ...
                for i, item in enumerate(active_list['items']):
                    if params in item.lower() or params == str(i + 1):
                        ...
                        for j in range(i, len(active_list['items'])):
                            ...
                            text_part = old_item.split('. ', 1)[1] if '. ' in old_item else old_item
            ...
            return ("Commands: 'add [item]', 'remove [item]', 'show', 'clear', 'tell'\n"
