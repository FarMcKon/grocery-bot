...
class TwilioSMSService:
    ...
    def __init__(self, do_inbound_server=False, dev_mode=True):
        ...
        if not all([self.account_sid, self.auth_token, self.from_number]):
            print(
                "Warning: Twilio credentials not configured. SMS sending will be disabled."
            )
            self.client = None
        else:
        ...
        if do_inbound_server and self.dev_mode:
            ...
            # set the SMS webhook
            ph_numbers = self.client.incoming_phone_numbers.list(
                phone_number=os.getenv("TWILIO_PHONE_NUMBER")
            )
            if not self.ngrok_url:
                print(
                    "Error: ngrok is not configured locally. SMS inbound server will not be started."
                )
            else:
        ...
        try:
            if self.client:
                for call in self.client.api.account.calls.list(limit=1):
                    print(
                        "Call sid " + call.sid + ": " + str(call.duration) + " seconds."
                    )
                return True
            else:
